		   ...
		   function IsNumeric(input){
            ...
            $(function(){
                ...
                $(".btnModal").click(function(){
                    var idAfiliacion=$(this).attr("id_obj");
					var categoriaID=$(this).attr("categoriaID");
					var clubID=$(this).attr("clubID");
					var deporteID=$(this).attr("deporteID");
					
					$variablesPase = {'idAfiliacion':idAfiliacion,'categoriaID':categoriaID,'clubID':clubID,'deporteID':deporteID};
					...
					if(relacionado=="seeObj"){
						...
						$("#respEditData").load("editarAfiliacion.php",$variablesPase);
					}else if(relacionado=="removeObj"){
						var r=window.confirm("¿Desea eliminar el objeto seleccionado?");
						if (r==true){
							$("#respDelete").load("eliminarAfiliacionProc.php",{'idAfiliacion':idAfiliacion});
							$(".fila").each(function(){
								if($(this).attr("id_obj")==idAfiliacion){
									$(this).remove();
								}
							})
							var total = 0;
				...
				$("#btnNuevo").click(function(){
					$("#respAddData").load("addAfiliacion.php");					         			
				});
...
<?php
	...
	/*
		...
		*/
		include "dbconex.php";
		$sql = "Select DCC.DeporteCategoriaClubID, DCC.CategoriaID, CA.Nombre as CategoriaNombre, DCC.ClubID, C.Nombre as ClubNombre, DCC.DeporteID, D.Nombre as DeporteNombre
				from DeporteCategoriaClub DCC, Club C, Categoria CA,Deporte D
				where DCC.CategoriaID = CA.CategoriaID AND DCC.ClubID = C.ClubID AND DCC.DeporteID = D.DeporteID
				ORDER BY DeporteNombre, CategoriaNombre, ClubNombre";
		$conn = conectarBD();
...
<div class="portlet box blue">
	<div class="portlet-title">
		<div class="caption" >
			<i class="icon-pencil"></i> Afiliaciones
        </div>
		...
		<table align="center" class="table  table-striped table-bordered">
			...
			<?php
				...
				while ($col = sqlsrv_fetch_array( $stmt, SQLSRV_FETCH_ASSOC)) {
					...
					$clubID = $col['ClubID'];
					$clubNombre = utf8_encode($col['ClubNombre']);
			?>
			<tr class="fila"  id_obj="<?php echo printVar($id); ?>">
				...
				<td align="center">						   
					...
					<a class="btnModal opci"  relacionado="editObj" 
						...
						clubID="<?php echo printVar($clubID); ?>"
						data-toggle="modal" 
