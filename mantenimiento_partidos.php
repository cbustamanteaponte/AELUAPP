		   ...
		   function IsNumeric(input){
            ...
            $(function(){
                ...
                $(".btnModal").click(function(){
                    var idPartido=$(this).attr("id_obj");
					var deporteID=$(this).attr("deporteID");
					var deporteNombre=$(this).attr("deporteNombre");
					var categoriaID=$(this).attr("categoriaID");
					var categoriaNombre=$(this).attr("categoriaNombre");
					var club1ID=$(this).attr("club1ID");
					var club1Nombre=$(this).attr("club1Nombre");
					var club2ID=$(this).attr("club2ID");
					var club2Nombre=$(this).attr("club2Nombre");
					var fecha=$(this).attr("fecha");
					var lugarID=$(this).attr("lugarID");
					var lugarNombre=$(this).attr("lugarNombre");
					
					$variablesPase = {'idPartido':idPartido, 'deporteID':deporteID,'deporteNombre':deporteNombre,'categoriaID':categoriaID,'categoriaNombre':categoriaNombre,'club1ID':club1ID,'club1Nombre':club1Nombre,'club2ID':club2ID,'club2Nombre':club2Nombre,'fecha':fecha,'lugarID':lugarID,'lugarNombre':lugarNombre};
					...
					if(relacionado=="seeObj"){
						...
						$("#respEditData").load("editarPartido.php",$variablesPase);
					}else if(relacionado=="removeObj"){
						var r=window.confirm("¿Desea eliminar el objeto seleccionado?");
						if (r==true){
							$("#respDelete").load("eliminarPartidoProc.php",{'idPartido':idPartido});
							$(".fila").each(function(){
								if($(this).attr("id_obj")==idPartido){
									$(this).remove();
								}
							})
							var total = 0;
				...
				$("#btnNuevo").click(function(){
					$("#respAddData").load("addPartido.php");					         			
				});
...
<?php
	...
	/*
		...
		*/
		include "dbconex.php";
		$sql = "select p.PartidoID, p.DeporteID, d.Nombre as deporteNombre, c.CategoriaID, c.Nombre as categoriaNombre,
				 c1.ClubID as club1ID, c1.Nombre as club1Nombre, c2.ClubID as club2ID, c2.Nombre as club2Nombre, l.LugarID, l.Nombre as lugarNombre, p.Fecha
				from partido p, deporte d, categoria c, club c1, club c2, lugar l
				where p.DeporteID = d.DeporteID and p.CategoriaID = c.CategoriaID and p.ClubID1 = c1.ClubID and p.ClubID2 = c2.ClubID and p.LugarID = l.LugarID and p.Activo='ACT'";
		$conn = conectarBD();
...
<div class="portlet box blue">
	<div class="portlet-title">
		<div class="caption" >
			<i class="icon-pencil"></i> Partidos
        </div>
	...
	<div class="portlet-body">
		...
		<table align="center" class="table  table-striped table-bordered">
			<tr>
				...
				<td align="center"><b>Deporte</b></td>
				<td align="center"><b>Categoría</b></td>
				<td align="center"><b>Club 1</b></td>
				<td align="center"><b>Club 2</b></td>
			...
			<?php
				...
				while ($col = sqlsrv_fetch_array( $stmt, SQLSRV_FETCH_ASSOC)) {
					$id = $col['PartidoID'];
					$deporteID = $col['DeporteID'];
					$deporteNombre = utf8_encode($col['deporteNombre']);
					$categoriaID = $col['CategoriaID'];
					$categoriaNombre = utf8_encode($col['categoriaNombre']);
					$club1ID = $col['club1ID'];
					$club1Nombre = utf8_encode($col['club1Nombre']);
					$club2ID = $col['club2ID'];
					$club2Nombre = utf8_encode($col['club2Nombre']);
					$lugarID = $col['LugarID'];
					$lugarNombre = utf8_encode($col['lugarNombre']);
					$fecha = date_format($col['Fecha'],'d M Y H:i');
			?>
			<tr class="fila"  id_obj="<?php echo printVar($id); ?>">
                <!--<td><?php echo printVar($id); ?></td>-->
				<td><?php echo $deporteNombre; ?></td>
				<td><?php echo $categoriaNombre; ?></td>
				<td><?php echo $club1Nombre; ?></td>
				<td><?php echo $club2Nombre; ?></td>
				<td><?php echo $lugarNombre; ?></td>
				<td><?php echo $fecha; ?></td>
				<td align="center">						   
					<a class="btnModal opci"  relacionado="editObj" 
						id_obj="<?php echo printVar($id); ?>" 
						deporteNombre="<?php echo printVar($deporteNombre); ?>"
						deporteID="<?php echo printVar($deporteID); ?>"
						categoriaNombre="<?php echo printVar($categoriaNombre); ?>"
						categoriaID="<?php echo printVar($categoriaID); ?>"
						club1Nombre="<?php echo printVar($club1Nombre); ?>"
						club1ID="<?php echo printVar($club1ID); ?>"
						club2Nombre="<?php echo printVar($club2Nombre); ?>"
						club2ID="<?php echo printVar($club2ID); ?>"
						lugarNombre="<?php echo printVar($lugarNombre); ?>"
						lugarID="<?php echo printVar($lugarID); ?>"
						fecha="<?php echo printVar($fecha); ?>"
						data-toggle="modal" 
