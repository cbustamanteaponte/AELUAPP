		   ...
		   function IsNumeric(input){
            ...
            $(function(){
                ...
                $(".btnModal").click(function(){
                    var idEvento=$(this).attr("id_obj");
					var eventoNombre=$(this).attr("eventoNombre");
					...
					$variablesPase = {'idEvento':idEvento, 'eventoNombre':eventoNombre,'descripcion':descripcion,'fechaInicio':fechaInicio,'fechaFin':fechaFin,'lugarID':lugarID,'costo':costo,'imagen':imagen,'logo':logo};
					...
					if(relacionado=="seeObj"){
						...
						$("#respEditData").load("editarEvento.php",$variablesPase);
					}else if(relacionado=="removeObj"){
						var r=window.confirm("¿Desea eliminar el objeto seleccionado?");
						if (r==true){
							$("#respDelete").load("eliminarEventoProc.php",{'idEvento':idEvento});
							$(".fila").each(function(){
								if($(this).attr("id_obj")==idEvento){
									$(this).remove();
								}
							})
							var total = 0;
				...
				$("#btnNuevo").click(function(){
					$("#respAddData").load("addEvento.php");					         			
				});
...
<?php
	...
	if (!isset($_SESSION["usuario"])){
		...
		include "dbconex.php";
		$sql = "select e.EventoID, e.Nombre as eventoNombre, e.Descripcion, e.FechaInicio, e.FechaFin, l.LugarID, l.Nombre as lugarNombre, e.Costo, e.Imagen, e.Thumbnail
				from Evento e, Lugar l
				where e.LugarID = l.LugarID and e.Activo='ACT';";
		$conn = conectarBD();
	...
	<div class="portlet-body">
		...
		<table align="center" class="table  table-striped table-bordered">
			<tr>
				...
				<td align="center"><b>Nombre Evento</b></td>
			...
			<?php
				...
				while ($col = sqlsrv_fetch_array( $stmt, SQLSRV_FETCH_ASSOC)) {
					$id = $col['EventoID'];
					$eventoNombre = utf8_encode($col['eventoNombre']);
					$descripcion = utf8_encode($col['Descripcion']);
					$fechaInicio = date_format($col['FechaInicio'],'d M Y H:i');
					$fechaFin = date_format($col['FechaFin'],'d M Y H:i');
					$lugarID = $col['LugarID'];
					$lugarNombre = utf8_encode($col['lugarNombre']);
					$costo = utf8_encode($col['Costo']);
			...
			<tr class="fila"  id_obj="<?php echo printVar($id); ?>">
				...
				<td align="center">						   
					<!--<a class="btnModal" 
						...
						titulo="<?php echo printVar($titulo); ?>"
					data-toggle="modal" href="#respSee"><i class="icon-search text"  content="Visualizar"></i></a>&nbsp;&nbsp;&nbsp;
					-->
					<a class="btnModal opci"  relacionado="editObj" 
						id_obj="<?php echo printVar($id); ?>" 
						eventoNombre="<?php echo printVar($eventoNombre); ?>"
						descripcion="<?php echo printVar($descripcion); ?>"
						fechaInicio="<?php echo printVar($fechaInicio); ?>"
						fechaFin="<?php echo printVar($fechaFin); ?>"
						lugarID="<?php echo printVar($lugarID); ?>"
						costo="<?php echo printVar($costo); ?>"
